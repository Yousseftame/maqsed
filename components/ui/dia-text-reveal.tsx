...
export function DiaTextReveal({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // If `once`, let the in-flight sweep finish — stopping it freezes
      // the text in a mostly-transparent mid-gradient state.
      if (once) return
      stopRef.current?.()
  ...
  return (
    <motion.span
      ref={spanRef}
      className={cn("inline-block align-bottom leading-[1.2] text-inherit", className)}
      style={{
        color: "transparent",
        ...
        backgroundImage,
        // Keep descenders / bold metrics from getting clipped by the text clip box
        paddingBlock: "0.06em",
        marginBlock: "-0.06em",
        ...(isMulti && {
          overflow: "hidden",
