...
export function EditUnitModal({ isOpen, onClose, unit }: EditUnitModalProps) {
  ...
  useEffect(() => {
    if (isOpen && unit) {
      reset({
        projectId: unit.projectId || "independent",
        cityId: unit.cityId || "",
        ...
        unitNumber: unit.unitNumber?.includes("-") ? unit.unitNumber.split("-").pop() : (unit.unitNumber || ""),
        unitType: unit.unitType || "شقة سكنية",
        operationType: unit.operationType || "للبيع",
        ...
        discountDays: unit.discountDays?.toString() || "",

        additionalComponents: unit.additionalComponents || [],
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      const originalUrls = unit.images || [];
      const deletedUrls = originalUrls.filter(url => !existingUrls.includes(url));

      if (deletedUrls.length > 0) {
        await Promise.all(deletedUrls.map(url => unitsService.deleteImage(url)));
      }
      
      const uploadedUrls = await Promise.all(
        newFiles.map((img) => unitsService.uploadImage(img.file as File, `units/${unit.id}`))
      );
      ...
      if (isIndependent) {
        ...
        // Remove independent fields if switching
        unitData.cityId = null;
      ...
      if (data.discountActive) {
        ...
        unitData.discountPercentage = null;
        unitData.discountDays = null;
      }

      await unitsService.updateUnit(unit.id, unitData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["units"] });
      toast.success(t("admin.ui.success") || "تم تحديث الوحدة بنجاح");
      onClose();
    },
    onError: (err: any) => {
      console.error(err);
      toast.error(err?.message || t("admin.ui.error") || "حدث خطأ أثناء التحديث");
    },
  });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 no-scrollbar sm:px-8 bg-[#FAF9F6]">
              <form id="edit-unit-form" onSubmit={handleSubmit(onSubmit)}>
